...
public class Tween // return DG.Tweening.Tween
{
    ...
    {
        DOTween.Sequence()
        .SetLink(impact.Transform.gameObject)
        .Append(impact.Transform.DOScale(new Vector3(1f - impact.Power, 1f + impact.Power * 2, 1f - impact.Power), impact.Time / 2))
        .Append(impact.Transform.DOScale(Vector3.one, impact.Time / 2));
    }
    ...
    public void Spawn(Spawn spawn)
    {
        ...
        spawn.Transform.DOScale(Vector3.one, spawn.Duration)
        .SetLink(spawn.Transform.gameObject)
        .SetEase(Ease.OutBack);
    }

    public void MoveToPosition(MoveToPosition moveToPosition)
    {
        moveToPosition.Transform.DOMove(moveToPosition.TargetPosition, moveToPosition.Duration)
        .SetLink(moveToPosition.Transform.gameObject)
        .SetEase(Ease.InOutSine);
    }

    public void Destroy(Destroy destroy)
    {
        destroy.Transform.DOScale(Vector3.zero, destroy.Duration)
        .SetLink(destroy.Transform.gameObject)
        .SetEase(Ease.InBack)
        .OnComplete(() => destroy.Transform.gameObject.SetActive(false));
    }
    #endregion

    #region SFX
    public DG.Tweening.Tween Shake(Transform transform, float duration = 0.3f, float strength = 10f)
    {
    ...
    public Sequence Blink(SpriteRenderer spriteRenderer, int blinks = 2, float blinkDuration = 0.1f)
    {
    ...
    public DG.Tweening.Tween Pulse(Transform transform, float scaleAmount = 1.1f, float speed = 0.6f, int count = 1) // pulses -1 будет бессконечностью
    {
    ...
    #region Text
    public Sequence FloatText(Transform transform, float moveY = 1f, float duration = 0.5f)
    {
    ...
    public DG.Tweening.Tween Fade(CanvasGroup group, float value, float duration = 0.5f, bool isActive = true)
    {
